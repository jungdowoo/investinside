...
Investinfo는 SEC EDGAR와 Form 13F 공식 공개 공시를 직접 수집해 유명 투자자와 관련된 운용사의 분기별 보유 내역을 보여주는 한국어 교육형 정보 서비스입니다.
...
SEC_USER_AGENT=Investinfo/1.0 contact: your-email@example.com
```
...
```powershell
pnpm seed:investors
pnpm seed:stock-aliases
pnpm sync:sec:browser
pnpm collect:company-facts
```

`seed:investors`는 사전에 지정된 CIK의 공식 기관명과 최근 13F 제출 여부를 SEC submissions 원문에서 다시 검증한 뒤에만 투자자 목록을 저장합니다. `seed:stock-aliases`는 검증된 CUSIP에만 ticker와 한글 종목명을 연결하고 기존 보유 데이터에 역적용합니다. `sync:sec:browser`는 `.env.local`의 Supabase 및 SEC 설정을 사용해 공식 SEC EDGAR 자료만 조회하고 Supabase에 저장합니다. `collect:company-facts`는 SEC XBRL companyfacts에서 종목별 재무 항목을 갱신합니다. `scripts/run-sec-sync.cmd`는 Windows 작업 스케줄러용 실행 파일이며 로그는 `logs/sec-sync.log`에 기록됩니다.
