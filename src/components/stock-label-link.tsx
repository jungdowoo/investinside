...
export function StockLabelLink({ ticker, issuerName, className = "" }: { ticker: string | null | undefined; issuerName: string; className?: string }) {
  ...
  return ticker ? <Link href={`/stocks/${ticker.toLowerCase()}`} className={`hover:text-amber-700 hover:underline ${className}`}>{label}</Link> : <span className={className}>{label}</span>;
}
