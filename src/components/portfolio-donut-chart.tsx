...
const COLORS = ["#0f766e", "#0284c7", "#4f46e5", "#7c3aed", "#be185d", "#e11d48", "#ea580c", "#ca8a04", "#65a30d", "#475569"];
...
export function PortfolioDonutChart({ holdings }: { holdings: Holding[] }) {
  ...
  if (otherWeight > 0.005) base.push({ key: "other", ticker: null, label: "기타 종목", weight: otherWeight, color: "#d6d3d1" });
  ...
  return <div className="mt-6 grid items-center gap-8 rounded-lg border border-stone-200 bg-white/80 p-5 shadow-sm sm:p-7 lg:grid-cols-[minmax(260px,.8fr)_1.2fr]">
    <div className="relative mx-auto w-full max-w-[340px]">
      <svg viewBox="0 0 200 200" role="img" aria-labelledby="portfolio-chart-title portfolio-chart-desc" className="h-auto w-full">
        ...
        <circle cx="100" cy="100" r="70" fill="none" stroke="#f5f5f4" strokeWidth="31" />
      ...
      <div className="pointer-events-none absolute inset-0 grid place-items-center text-center"><div className="max-w-32"><p className="line-clamp-2 text-sm font-black leading-5 text-stone-950">{active?.label ?? `${sorted.length}개 종목`}</p><p className="mt-1 text-xs font-bold text-teal-800">{active ? formatPercent(active.weight) : "마우스를 올려보세요"}</p></div></div>
    </div>
    <ol className="grid gap-x-6 gap-y-2 sm:grid-cols-2" aria-label="포트폴리오 구성 범례">{slices.map((slice, index) => <li key={slice.key} onMouseEnter={() => setHovered(slice.key)} onMouseLeave={() => setHovered(null)}>{slice.ticker ? <Link href={`/stocks/${slice.ticker.toLowerCase()}`} className="flex min-w-0 items-center gap-3 rounded-lg px-2 py-2 text-sm hover:bg-stone-50"><Legend slice={slice} index={index} /></Link> : <div className="flex min-w-0 items-center gap-3 px-2 py-2 text-sm"><Legend slice={slice} index={index} /></div>}</li>)}</ol>
  </div>;
}

function Legend({ slice, index }: { slice: Slice; index: number }) { return <><span className="size-3 shrink-0 rounded-full" style={{ backgroundColor: slice.color }} /><span className="min-w-0 flex-1 truncate font-bold"><span className="mr-1 text-stone-400">{slice.key === "other" ? "" : `${index + 1}.`}</span>{slice.label}</span><span className="shrink-0 font-mono text-xs text-stone-500">{formatPercent(slice.weight)}</span></>; }
