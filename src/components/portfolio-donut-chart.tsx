...
export function PortfolioDonutChart({ holdings }: { holdings: Holding[] }) {
  ...
  return <div className="mt-6 grid items-center gap-8 rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 shadow-sm p-5 sm:p-7 lg:grid-cols-[minmax(260px,.8fr)_1.2fr]">
    <div className="relative mx-auto w-full max-w-[340px]">
      ...
      <div className="pointer-events-none absolute inset-0 grid place-items-center text-center"><div className="max-w-32"><p className="line-clamp-2 text-sm font-bold leading-5 text-zinc-900 dark:text-zinc-100">{active?.label ?? `${sorted.length}개 종목`}</p><p className="mt-1 text-xs font-medium text-amber-700">{active ? formatPercent(active.weight) : "마우스를 올려보세요"}</p></div></div>
    </div>
    <ol className="grid gap-x-6 gap-y-2 sm:grid-cols-2" aria-label="포트폴리오 구성 범례">{slices.map((slice, index) => <li key={slice.key} onMouseEnter={() => setHovered(slice.key)} onMouseLeave={() => setHovered(null)}>{slice.ticker ? <Link href={`/stocks/${slice.ticker.toLowerCase()}`} className="flex min-w-0 items-center gap-3 rounded px-2 py-2 text-sm hover:bg-zinc-50 dark:hover:bg-zinc-800 dark:hover:bg-zinc-100 dark:bg-zinc-800/50"><Legend slice={slice} index={index} /></Link> : <div className="flex min-w-0 items-center gap-3 px-2 py-2 text-sm"><Legend slice={slice} index={index} /></div>}</li>)}</ol>
  </div>;
}

function Legend({ slice, index }: { slice: Slice; index: number }) { return <><span className="size-3 shrink-0 rounded-full" style={{ backgroundColor: slice.color }} /><span className="min-w-0 flex-1 truncate font-medium"><span className="mr-1 text-zinc-400">{slice.key === "other" ? "" : `${index + 1}.`}</span>{slice.label}</span><span className="shrink-0 font-mono text-xs text-zinc-500 dark:text-zinc-400">{formatPercent(slice.weight)}</span></>; }
